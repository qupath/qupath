...
public class TiffChecker {
	...
	final private static Logger logger = LoggerFactory.getLogger(TiffChecker.class);
	...
	static ImageCheckType checkTiff(final String path) {
		...
		TiffChecker.ImageCheckType type = null;
		try {
	...
	static ImageCheckType checkBigTiff(final RandomAccessFile in, final boolean littleEndian) throws IOException {
		...
		logger.warn("Checking Big TIFF images currently not supported!!!");
