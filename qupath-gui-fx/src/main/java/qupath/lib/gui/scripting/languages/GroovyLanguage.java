 ...
 * @since v0.4.0
 */
public class GroovyLanguage extends ScriptLanguage implements RunnableLanguage {
	
	final static private Logger logger = LoggerFactory.getLogger(GroovyLanguage.class);
	...
	public GroovyLanguage() {
		...
		this.name = "Groovy";
		this.exts = new String[] {".groovy"};
		this.syntax = GroovySyntax.getInstance();
		this.completor = new GroovyAutoCompletor();	
	...
	public Object executeScript(String script, Project<BufferedImage> project, ImageData<BufferedImage> imageData, boolean importDefaultMethods, ScriptContext context) throws ScriptException {
		...
		var engine = ScriptLanguageProvider.getEngineByName(name);
