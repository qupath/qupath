...
public class PathAwtIO {
	...
	public static void writeTMAData(File file, final ImageData<BufferedImage> imageData, OverlayOptions overlayOptions, final double downsampleFactor) {
		...
		if (outputCoreImages) {
			...
			final double downsample = Double.isNaN(downsampleFactor) ? (server.hasPixelSizeMicrons() ? ServerTools.getDownsampleFactor(server, PathPrefs.getPreferredTMAExportPixelSizeMicrons(), true) : 1) : downsampleFactor;
